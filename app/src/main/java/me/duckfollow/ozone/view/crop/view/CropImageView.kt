...
//import com.prasit.theboxshop.view.crop.task.BitmapCropTask
import me.duckfollow.ozone.view.crop.util.CubicEasing
import me.duckfollow.ozone.view.crop.util.RectUtils
...
open class CropImageView @JvmOverloads constructor(
    ...
    var targetAspectRatio: Float
        ...
        set(targetAspectRatio) {
            ...
            if (targetAspectRatio == SOURCE_IMAGE_ASPECT_RATIO) {
                mTargetAspectRatio =
                    drawable.getIntrinsicWidth() / drawable.getIntrinsicHeight().toFloat()
            } else {
                mTargetAspectRatio = targetAspectRatio
            }
     ...
     */
    fun cropAndSaveImage(compressFormat: Bitmap.CompressFormat, compressQuality: Int, cropCallback: BitmapCropCallback) {
        cancelAllAnimations()
        setImageToWrapCropBounds(false)

        val imageState = ImageState(mCropRect, RectUtils.trapToRect(mCurrentImageCorners), currentScale, currentAngle)
    ...
    fun setCropRect(cropRect: RectF) {
        ...
        mCropRect.set(
            cropRect.left - getPaddingLeft(), cropRect.top - getPaddingTop(),
     ...
     */
    protected override fun onImageLaidOut() {
        super.onImageLaidOut()
        val drawable = getDrawable() ?: return

        val drawableWidth = drawable.getIntrinsicWidth().toFloat()
        val drawableHeight = drawable.getIntrinsicHeight().toFloat()
        ...
        if (height > mThisHeight) {
            ...
            mCropRect.set(halfDiff.toFloat(), 0f, (width + halfDiff).toFloat(),
                mThisHeight.toFloat()
            )
        } else {
 ...
 * Runnable can be terminated either vie [.cancelAllAnimations] method
 * or when certai
n conditions inside [ZoomImageToPosition.run] method are triggered.
     */
    ...
    companion object {
        ...
        val DEFAULT_MAX_BITMAP_SIZE = 0
        val DEFAULT_IMAGE_TO_CROP_BOUNDS_ANIM_DURATION = 500
        val DEFAULT_MAX_SCALE_MULTIPLIER = 10.0f
        val SOURCE_IMAGE_ASPECT_RATIO = 0f
        val DEFAULT_ASPECT_RATIO = SOURCE_IMAGE_ASPECT_RATIO
    }
