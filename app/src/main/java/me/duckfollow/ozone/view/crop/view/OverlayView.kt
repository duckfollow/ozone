...
class OverlayView @JvmOverloads constructor(
    ...
    private fun getCurrentTouchIndex(touchX: Float, touchY: Float): Int {
        ...
        while (i < 8) {
            val distanceToCorner = Math.sqrt(
                Math.pow(
                    (touchX - mCropGridCorners[i]).toDouble(),
                    2.0
    ...
    companion object {
        ...
        val DEFAULT_SHOW_CROP_FRAME = true
        val DEFAULT_SHOW_CROP_GRID = true
        val DEFAULT_CIRCLE_DIMMED_LAYER = false
        val DEFAULT_FREESTYLE_CROP_MODE = FREESTYLE_CROP_MODE_DISABLE
        val DEFAULT_CROP_GRID_ROW_COUNT = 2
        val DEFAULT_CROP_GRID_COLUMN_COUNT = 2
    }
