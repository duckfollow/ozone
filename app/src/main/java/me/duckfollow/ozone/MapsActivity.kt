...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener,
    ...
    lateinit var image_animation:ImageView
    ...
    fun setWeather() {
        val rainy = UserProfile(this).getWeatherCode() in arrayOf("900","200", "201", "202","230","231","232","233","300","301","302","500","501","502","511","520","521","522","600","601","602","610","611","612","621","622","623")

        if (rainy) {
            image_animation.setBackgroundResource(R.drawable.rainy)
            this.image_animation.post {
                val anim = image_animation.background as AnimationDrawable
                anim.start()
            }
        } else {
            image_animation.setBackgroundResource(R.drawable.clouds)
            this.image_animation.post {
                val anim = image_animation.background as AnimationDrawable
                anim.start()
            }
    ...
    fun showProfile() {
        TapTargetView.showFor(this,
            TapTarget.forView(
                btn_profile,
                "โปรไฟล์ของคุณ",
                "คุณสามารถแก้ไขรูป และแสกนเพิ่มเพื่อนได้"
            ).cancelable(false),
    ...
    fun showMenuLocation () {
        TapTargetView.showFor(this,
            TapTarget.forView(
                btn_show_location,
                "ตำแหน่งปัจจุบัน",
                "คุณสามารถดูตำแหน่งปัจจุบันของคุณได้ และสามารถลาก เพื่อย้ายตำแหน่ง"
            ).cancelable(false),
            object : TapTargetView.Listener() {
                override fun onTargetClick(view: TapTargetView) {
                    super.onTargetClick(view)
//                    showList()
    ...
    fun showList() {
        TapTargetView.showFor(this,
            TapTarget.forView(
                btn_menu,
                "ข้อมูลเพิ่มเติม",
                "คุณสามารถดูรายการทั้งหมดที่แสดงอยู่บนแผนที่ได้"
            ).cancelable(false),
            object : TapTargetView.Listener() {
                override fun onTargetClick(view: TapTargetView) {
                    super.onTargetClick(view)
    ...
    fun showViewMap(){
        TapTargetView.showFor(this,
            TapTarget.forView(viewMap, "คลิกที่หมุด", "คลิกเพื่อแสดงรายละเอียดต่างๆ").cancelable(
                false
            ),
            object : TapTargetView.Listener() {
                override fun onTargetClick(view: TapTargetView) {
    ...
    private fun initView(){
        ...
        image_animation = findViewById(R.id.image_animation)
    }
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        try {
            val view_compass = (viewMap.findViewById<View>(Integer.parseInt("1")).parent as View).findViewById<View>(
                Integer.parseInt(
                    "5"
                )
            )
...
//                UserProfile(this@MapsActivity).setImageBase64(img)
                ...
                try {
                    ...
                    val imgBitmap = getCroppedBitmap(
                        ConvertImagetoBase64().getResizedBitmap(
                            b,
                            150,
                            150
                        )
...
fun getCroppedBitmap(bitmap: Bitmap):Bitmap {
    ...
    private fun getMarkerBitmapFromView ():Bitmap {
        val customMarkerView = (getSystemService(LAYOUT_INFLATER_SERVICE) as LayoutInflater).inflate(
            R.layout.marker,
            null
        )
    ...
    override fun onConnected(p0: Bundle?) {
            val locationAvailability = LocationServices.FusedLocationApi.getLocationAvailability(
                googleApiClient
            )
            try {
    ...
    override fun onLocationChanged(p0: Location?) {
        ...
        if(UserProfile(this).getImageBase64() != "" && UserProfile(this).getImageBase64() != "null"){
        ...
        val l1 = LatLng(UserProfile(this).getMyLat().toDouble(),UserProfile(this).getMyLong().toDouble())
        val l2 = LatLng(mLat,mLong)

        val distance = getDistance(l1,l2)

        if (distance / 1000.0f > 10) {
            UserProfile(this).setMyLat(mLat.toString())
            UserProfile(this).setMyLong(mLong.toString())
            TaskWeatherData().execute("https://weatherbit-v1-mashape.p.rapidapi.com/current?lon=${mLong}&lat=${mLat}")
        }

        Log.d("distanceMyLocation", distance.toString())
    }

    private fun getDistance(my_latlong: LatLng, frnd_latlong: LatLng): Float {
        val l1 = Location("One")
...
//                review()
    ...
    inner class TaskWeatherData:AsyncTask<String, String, String>(){
        override fun doInBackground(vararg params: String?): String? {
            return ApiConnection().getDataWeather(params[0].toString())
        }
        ...
        override fun onPostExecute(result: String?) {
            ...
            try {
                ...
                val json = JSONObject(data2[0].toString())

                val code = json.getJSONObject("weather").getString("code")
                UserProfile(this@MapsActivity).setWeatherCode(code)
                Log.d("DATAWEATHER", json.getJSONObject("weather").getString("code"))
